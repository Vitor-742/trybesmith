...
const validateClasse = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!classe) {
    return res
      .status(400).json({ message: '"classe" is required' });
  }
  if (typeof classe !== 'string') {
    return res
      .status(422).json({ message: '"classe" must be a string' });
  }
  if (classe.length < 3) {
    return res
...
userRouter.post(
  ...
  async (req: Request, res: Response) => {
    await createUser(req.body);// jwt
    const token = jwt.sign(req.body, '1234');
